        ...
        if (!email || !password) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    window.location.href = `{{ route('employee.web.authentication.oAuth') }}?token=${response.response.token}&remember=${remember}`;
                },
                error: function (error) {
                    ...
                    if (error.status === 422) {
                        ...
                        toastr.error('Personel Bulunamadı.');
